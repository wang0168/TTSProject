...
import tts.moudle.huanxinapi.utils.CameraHelper;
...
public class VideoCallActivity extends CallActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		TTSBaseApplication.getInstance().isVideoCalling = true;
	...
	protected void onDestroy() {
		super.onDestroy();
		TTSBaseApplication.getInstance().isVideoCalling = false;
		stopMonitor();
		try {
		...
		new Thread(new Runnable() {
			public void run() {
				while (monitor) {
					runOnUiThread(new Runnable() {
						public void run() {
							monitorTextView.setText("宽x高：" + callHelper.getVideoWidth() + "x"
