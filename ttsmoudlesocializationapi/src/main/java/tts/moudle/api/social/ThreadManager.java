...
public final class ThreadManager {
	...
	/**
	 * AsyncTask的默认线程池Executor. 负责长时间的任务(网络访问) 默认3个线�?	 */
	public static final Executor NETWORK_EXECUTOR;

	/**
	 * 副线程的Handle, 只有�?��线程 可以执行比较快但不能在ui线程执行的操�? 文件读写不建议在此线程执�?
	 * 请使用FILE_THREAD_HANDLER 此线程禁止进行网络操�?如果�?��进行网络操作. 请使用NETWORK_EXECUTOR
	 */
	...
	/**
	 * 文件读写线程的Handle, 只有�?��线程 可以执行文件读写操作, 如图片解码等 此线程禁止进行网络操�?如果�?��进行网络操作.
	 * 请使用NETWORK_EXECUTOR
	 */
	...
	private static Executor initNetworkExecutor() {
		...
		if (result instanceof ThreadPoolExecutor) {
			// core size减少�?�?			((ThreadPoolExecutor) result).setCorePoolSize(3);
		}
	...
	/**
	 * 在网络线程上执行异步操作. 该线程池负责网络请求等操�?长时间的执行(如网络请求使用此方法执行) 当然也可以执行其�?线程和AsyncTask公用
	 * 
	 * @param run
	 */
	...
	/**
	 * 获得文件线程的Handler.<br>
	 * 副线程可以执行本地文件读写等比较快但不能在ui线程执行的操�?<br>
	 * <b>此线程禁止进行网络操�?如果�?��进行网络操作. 请使用NETWORK_EXECUTOR</b>
	 * 
	 * @return handler
	 */
	...
	/**
	 * 获得副线程的Handler.<br>
	 * 副线程可以执行比较快但不能在ui线程执行的操�?<br>
	 * 另外, 文件读写建议放到FileThread中执�?<b>此线程禁止进行网络操�?如果�?��进行网络操作.
