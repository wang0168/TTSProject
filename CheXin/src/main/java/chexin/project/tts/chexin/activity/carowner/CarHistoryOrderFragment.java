 ...
 */
public class CarHistoryOrderFragment extends BaseFragment {
    ...
    private void adapterCar() {
        ...
        orderListAdapter = new CarOrderListAdapter(getActivity(), mData, 2);
        ...
        orderListAdapter.setActionOnClickListener(new CarOrderListAdapter.ActionOnClickListener() {
            ...
            public void doAction(View v, int postion) {
                ...
                startActivity(new Intent(getActivity(), AssessmentActivity.class).
                        putExtra("from", "2").putExtra("order_id", order_id).putExtra("face", mData.get(postion).
                        getMemberBean().getHead_path()).putExtra("name", mData.get(postion).getMemberBean().getName()));
            }
        });
    }
    ...
    protected void startRequestData(int index) {
        ...
        switch (index) {
            case getData:
                ...
                params.put("type", "2");
                params.put("page", "1");
                getDataWithPost(getData, Host.hostUrl + "car.api?getCarOrders", params);
                break;
            case loadMore:
                params = new ArrayMap<>();
                params.put("UserId", MyAccountMoudle.getInstance().getUserInfo().getId() + "");
                ...
                getDataWithPost(loadMore, Host.hostUrl + "car.api?getCarOrders", params);
                break;
            case done:
                params = new ArrayMap<>();
                params.put("UserId", MyAccountMoudle.getInstance().getUserInfo().getId() + "");
